...
export function CartPage() {
  ...
  return (
    <div className="wrap" style={{ maxWidth: 'var(--max)', margin: '0 auto', padding: '2rem' }}>
      <div className="mb-6">
        <h1 className="font-display text-4xl mb-2">Shopping Cart</h1>
        <p className="text-ink-muted">{itemCount} {itemCount === 1 ? 'item' : 'items'}</p>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="flex gap-4 p-6 bg-lavender-50 rounded-lg">
                {/* Product Image */}
                  ...
                  <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className="w-10 h-10 flex items-center justify-center border border-line rounded-lg hover:bg-lavender-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
