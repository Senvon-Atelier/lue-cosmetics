...
// newMergeRouter builds a full chi router with auth + cart public + a
// RequireSession group exposing cart's auth-gated /cart/merge. Mirrors
// cmd/api/main.go's mount pattern.
func newMergeRouter(t *testing.T) (http.Handler, *pgxLike, func()) {
	t.Helper()
	ctx, pool, cleanup := testsupport.StartPool(t, "../../migrations")
	logger := zap.NewNop()
	...
	return r, &pgxLike{ctx: ctx, pool: pool}, cleanup
}

// pgxLike holds the integration test's pool + ctx so callers can seed.
type pgxLike struct {
	ctx  context.Context
	pool db.Pool
}

func signupGetCookie(t *testing.T, router http.Handler, email string) *http.Cookie {
	t.Helper()
	rr := doJSON(t, router, http.MethodPost, "/auth/signup", map[string]string{
		"email": email, "password": "hunter22", "name": "Test",
	})
	if rr.Code != http.StatusCreated {
		t.Fatalf("signup(%s) = %d; %s", email, rr.Code, rr.Body.String())
	}
	c := testsupport.FindCookie(rr.Result(), "rue_session")
	if c == nil {
		t.Fatalf("signup(%s): no session cookie", email)
	}
	return c
}

func TestHandler_MergeCart_Happy(t *testing.T) {
	router, h, cleanup := newMergeRouter(t)
	defer cleanup()
	productID := seedTestProduct(t, h.ctx, h.pool)

	// Step 1: create a guest cart with one item.
	rrAdd := doJSON(t, router, http.MethodPost, "/cart/items", map[string]any{
		"product_id": productID.String(), "qty": 2,
	})
	if rrAdd.Code != http.StatusOK {
		t.Fatalf("guest add = %d; %s", rrAdd.Code, rrAdd.Body.String())
	}
	guestCookie := testsupport.FindCookie(rrAdd.Result(), GuestCookieName)
	if guestCookie == nil {
		t.Fatal("no guest cookie minted")
	}
	guestToken := guestCookie.Value

	// Step 2: signup → session cookie.
	sessionCookie := signupGetCookie(t, router, "happy@merge.test")

	// Step 3: POST /cart/merge with both cookies + token in body.
	rr := doJSON(t, router, http.MethodPost, "/cart/merge", map[string]any{
	...
	if len(body.Items) != 1 || body.Items[0].Qty != 2 {
		t.Errorf("merged cart items = %+v, want 1 qty=2", body.Items)
	}

	// Verify guest cookie cleared.
	cleared := testsupport.FindCookie(rr.Result(), GuestCookieName)
	if cleared == nil {
		t.Fatal("expected guest cookie clear (Max-Age=0) in response")
	}
	if cleared.MaxAge >= 0 || cleared.Value != "" {
		t.Errorf("guest cookie should be cleared; got value=%q maxage=%d", cleared.Value, cleared.MaxAge)
	}
}

func TestHandler_MergeCart_Unauthenticated_401(t *testing.T) {
	router, _, cleanup := newMergeRouter(t)
	defer cleanup()

	rr := doJSON(t, router, http.MethodPost, "/cart/merge", map[string]any{
		"guest_token": "irrelevant",
	})
	if rr.Code != http.StatusUnauthorized {
		t.Errorf("merge unauth = %d, want 401; body: %s", rr.Code, rr.Body.String())
	}
}

func TestHandler_MergeCart_UnknownGuestToken_200(t *testing.T) {
	router, _, cleanup := newMergeRouter(t)
	defer cleanup()

	sessionCookie := signupGetCookie(t, router, "unknown-tok@merge.test")

	rr := doJSON(t, router, http.MethodPost, "/cart/merge", map[string]any{
		"guest_token": "00000000-0000-0000-0000-0000deadbeef",
	}, sessionCookie)
	if rr.Code != http.StatusOK {
		t.Fatalf("merge w/ unknown token = %d; body: %s", rr.Code, rr.Body.String())
	}
	var body cartResponse
	if err := json.NewDecoder(rr.Body).Decode(&body); err != nil {
		t.Fatalf("decode: %v", err)
	...
	if len(body.Items) != 0 {
		t.Errorf("expected empty user cart, got %+v", body.Items)
	}
}
